...
// FundX Pro — the monetization surface (C-113 §7: Premium Pool Creation / verified
// pool tier). A real Pi User-to-App payment; this also satisfies the Pi Portal
// "Process a Transaction" checklist step. ADR-007 dual-mode: Hub navigation →
// Mode 1 (Hub modal); standalone in Pi Browser → Mode 2 (direct createU2APayment).
// Approves under PI_API_KEY_FUNDX (never the default Hub key — C-12 §11).
//
// NOTE: this is a SUBSCRIPTION, NOT a pool contribution. FundX moves no pooled
// capital in the client and computes no returns (C-113 §6) — pool escrow lives in
// tec-payment-service, gated behind legal review + KYC + SYSTEM approval (§11 P0).
...
const PRICE   = 20;                     // π / month
const ITEM_ID = 'fundx_pro_monthly';
const MEMO    = 'TEC FundX — Pro (1 month)';
...
export function FundXPro() {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      setStatus('paying');
      const result = await createU2APayment(PRICE, MEMO, { item_id: ITEM_ID, plan: 'fundx_pro' }, internalId);
  ...
  if (status === 'success') {
    return (
      <div style={{ ...card, borderColor: `${TEC_COLORS.success}66` }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.success }}>✅ FundX Pro active</div>
        <div style={{ fontSize: 12, color: TEC_COLORS.subtext, marginTop: 6 }}>
          Payment received. Thanks for supporting TEC FundX 📈.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={card}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.gold }}>📈 FundX Pro</div>

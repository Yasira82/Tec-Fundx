...
import { Icon, type FxIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type FxTab = 'home' | 'pools' | 'pro' | 'settings';

export function BottomNav({ active, onSelect }: { active: FxTab; onSelect: (t: FxTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: FxTab; icon: FxIconName; label: string }[] = [
    { key: 'home',     icon: 'coins',    label: t.fundx.nav.home     },
    { key: 'pools',    icon: 'layers',   label: t.fundx.nav.pools    },
    { key: 'pro',      icon: 'star',     label: t.fundx.nav.pro      },
    { key: 'settings', icon: 'settings', label: t.fundx.nav.settings },
  ];
  return (
    <nav style={{
